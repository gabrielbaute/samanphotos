...
def detect_faces(photo):
    ...
    try:
        ...
        current_app.logger.error(f"Error al procesar la imagen {photo.path}: {e}")
        return [], []
...
def process_encodings(photo, face_encodings, face_locations):
    ...
    try:
        try:
            ...
            current_app.logger.error(f"Error al obtener codificaciones existentes: {e}")
        ...
        current_app.logger.error(f"Error al procesar las codificaciones faciales: {e}")
...
def process_photo(photo):
    ...
    try:
        ...
        current_app.logger.error(f"Error al procesar la foto {photo.path}: {e}")
        raise e
...
def comparefaces(target_encoding, tolerance=0.9):
    ...
    try:
        ...
        current_app.logger.error(f"Error al comparar rostros: {e}")
        return []